...
class AuthitemController extends SBaseController {
  ...
  public function actionAssign() {
	...
	$assBizRuleDB=array();
	$allDBConnectControlAction=HelpTool::dbConnectControlAction();
	foreach($allDBConnectControlAction as $connectionKey=>$connectionVal)
	{
		$assBizRuleDB[$connectionKey]=isset($_POST['Assignments']['bizrule'][$connectionKey]) ? $_POST['Assignments']['bizrule'][$connectionKey] : "";
	}
	
	$assBizRule=json_encode($assBizRuleDB);
  ...
  public function actionUpdatedb() {
	 ...
	 if(isset($_POST['bizrule']))
	{
		...
		foreach($allDBConnectControlAction as $connectionKey=>$connectionVal)
		{
			$assBizRuleDB[$connectionKey]=isset($_POST['bizrule'][$connectionKey]) ? $_POST['bizrule'][$connectionKey] : "";
		}
		...
		$sql="update $itemTableName set bizrule='".$assBizRule."' where name='".$thisName."'";
		$command = $connection->createCommand($sql);
		$command->execute();
	}
	 $this->renderPartial('manage/updatedb', array('model' => $model));
  }
