...
function select(){
	with(document.manage_form.province){
		var loca2 = options[selectedIndex].value; 
	}
	//var loca2 = document.getElementById('province').value; 
	for(i = 0;i < where.length;i ++){
		if (where[i].loca == loca2){
			loca3 = (where[i].locacity).split("|");
			for(j = 0;j < loca3.length;j++){
				with(document.manage_form.city){ 
...
function init(){
	var province = arguments[0] ? arguments[0] : '未选择省份';
  	var city = (arguments[1] && arguments[1].length !== 0) ? arguments[1] : '未选择城市';
  	var isSelect = (arguments[3]) ? arguments[1] : '未选择城市';
	with(document.manage_form.province){
		length = where.length;
